...
function v6AggregateOpportunities_(rows){
  ...
  (rows||[]).forEach(function(row){
    var amOwner=String(row.amOwner||'Unassigned'),type=String(row.opportunityType||'Unknown'),service=String(row.service||'Unspecified'),window=String(row.qnbWindow||row.window||''),key=[amOwner,type,service,window].join('\u001f'),suppressed=String(row.eligibilityStatus||'').toUpperCase()==='SUPPRESSED',run=String(row.lastEngineRun||row.updatedAt||row.detectedAt||'');
    if(!grouped[key])grouped[key]={amOwner:amOwner,opportunityType:type,service:service,window:window,detectedAccounts:0,eligibleAccounts:0,suppressedAccounts:0,priority:v6OpportunityPriority_(type),source:String(row.sourceReport||row.source||'PRIVATE COMMERCIAL REPORTS'),campaignStatus:String(row.campaignStatus||'OPPORTUNITY DETECTED'),nextAction:String(row.nextAction||'Evaluate campaign eligibility'),lastEngineRun:run||null};
    var group=grouped[key];group.detectedAccounts+=1;totalSignals+=1;
    if(suppressed){group.suppressedAccounts+=1;suppressedAccounts+=1;}else{group.eligibleAccounts+=1;eligibleAccounts+=1;}
    if(run&&(!group.lastEngineRun||run>group.lastEngineRun))group.lastEngineRun=run;if(run>lastEngineRun)lastEngineRun=run;
    families[v6OpportunityFamily_(type)]=true;byType[type]=(byType[type]||0)+1;
  });
  var groups=Object.keys(grouped).map(function(key){return grouped[key];}).sort(function(a,b){return a.priority-b.priority||b.eligibleAccounts-a.eligibleAccounts;});
  groups.forEach(function(group,index){group.groupId='OPP-GROUP-'+(index+1);});
  return {groups:groups,summary:{totalSignals:totalSignals,eligibleAccounts:eligibleAccounts,suppressedAccounts:suppressedAccounts,campaignFamilies:Object.keys(families).length,groupCount:groups.length,byType:byType,lastEngineRun:lastEngineRun||null}};
}
