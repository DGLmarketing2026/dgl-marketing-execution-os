...
function v6ContactAccountId_(record,accounts){
  ...
  if(found&&found.accountId)return v6ContactText_(found.accountId);
...
function v6IngestAuthoritativeContacts_(payload){
  ...
  accounts.forEach(function(input){
    try{
      var raw=Object.assign({},input||{},{sourceSystem:source}),accountId=v6ContactAccountId_(raw,accountRows),old=v6ContactExisting_(accountRows,['accountId'],{accountId:accountId})||{},external=v6ContactText_(raw.externalAccountId||raw.salesforceAccountId),record={accountId:accountId,externalSystem:source,externalAccountId:external,salesforceAccountId:source==='SALESFORCE'?external:v6ContactText_(old.salesforceAccountId),accountName:v6ContactText_(raw.accountName||old.accountName),amOwner:v6ContactText_(raw.amOwner||old.amOwner),status:v6ContactText_(raw.status||old.status||'ACTIVE').toUpperCase(),sourceUpdatedAt:v6ContactText_(raw.sourceUpdatedAt||old.sourceUpdatedAt),createdAt:v6ContactText_(old.createdAt||now),updatedAt:now};
  ...
  contacts.forEach(function(input){
    try{
      var raw=Object.assign({},input||{},{sourceSystem:source}),externalAccount=v6ContactText_(raw.externalAccountId||raw.salesforceAccountId),accountId=v6ContactText_(raw.accountId)||accountMap[source+'|'+externalAccount]||v6ContactAccountId_(raw,accountRows),contactId=v6ContactId_(raw,accountId),email=v6ContactEmail_(raw.email),valid=!!email&&v6ContactEmailValid_(email),dnc=v6ContactBool_(raw.doNotContact||raw.dnc),existing=v6ContactExisting_(v6ContactRowsSafe_('MKT_CONTACTS_SECURE'),['contactId'],{contactId:contactId})||{},record={contactId:contactId,accountId:accountId,externalSystem:source,externalContactId:v6ContactText_(raw.externalContactId||raw.salesforceContactId),salesforceContactId:source==='SALESFORCE'?v6ContactText_(raw.externalContactId||raw.salesforceContactId):v6ContactText_(existing.salesforceContactId),email:email,emailStatus:!email?'MISSING':valid?'VALID':'INVALID',doNotContact:dnc,status:v6ContactText_(raw.status||existing.status||'ACTIVE').toUpperCase(),sourceUpdatedAt:v6ContactText_(raw.sourceUpdatedAt||existing.sourceUpdatedAt),createdAt:v6ContactText_(existing.createdAt||now),updatedAt:now};
