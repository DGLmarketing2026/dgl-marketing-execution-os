...
// Canonical ID Bridge fix: real authoritative extracts arrive with externalSystem values
// like 'SALESFORCE_EXPORT', not the bare literal 'SALESFORCE'. The previous exact-match
// gate (source==='SALESFORCE') silently left salesforceAccountId/salesforceContactId
// empty for every real-world Salesforce-derived record even though externalAccountId/
// externalContactId were always populated. This recognizes any externalSystem whose
// uppercased value contains the word SALESFORCE (covers 'SALESFORCE', 'SALESFORCE_EXPORT',
// and reasonable future variants) without hardcoding a single literal as the only valid one.
function v6ContactIsSalesforceSource_(source){return /SALESFORCE/.test(v6ContactText_(source).toUpperCase());}
...
function v6IngestAuthoritativeContacts_(payload){
  ...
  accounts.forEach(function(input){
    try{
      var raw=Object.assign({},input||{},{sourceSystem:source}),accountId=v6ContactAccountId_(raw,accountRows),old=v6ContactExisting_(accountRows,['accountId'],{accountId:accountId})||{},external=v6ContactText_(raw.externalAccountId||raw.salesforceAccountId),resolvedSalesforceAccountId=v6ContactIsSalesforceSource_(source)?(external||v6ContactText_(old.salesforceAccountId)):v6ContactText_(old.salesforceAccountId),owner=v6ContactText_(raw.amOwner||raw.accountManager||old.amOwner||old.accountManager),record=Object.assign({},old,{accountId:accountId,externalSystem:source,externalAccountId:external||v6ContactText_(old.externalAccountId),salesforceAccountId:resolvedSalesforceAccountId,canonicalSalesforceIdStatus:resolvedSalesforceAccountId?'RESOLVED':'UNRESOLVED',accountName:v6ContactText_(raw.accountName||old.accountName),amOwner:owner,status:v6ContactText_(raw.status||old.status||'ACTIVE').toUpperCase(),sourceUpdatedAt:v6ContactText_(raw.sourceUpdatedAt||old.sourceUpdatedAt),createdAt:v6ContactText_(old.createdAt||now),updatedAt:now});
  ...
  contacts.forEach(function(input){
    try{
      var raw=Object.assign({},input||{},{sourceSystem:source}),externalAccount=v6ContactText_(raw.externalAccountId||raw.salesforceAccountId),accountId=accountMap[source+'|'+externalAccount]||v6ContactAccountId_(raw,accountRows),contactId=v6ContactId_(raw,accountId),existing=v6ContactExisting_(v6ContactRowsSafe_('MKT_CONTACTS_SECURE'),['contactId'],{contactId:contactId})||{},email=v6ContactEmail_(Object.prototype.hasOwnProperty.call(raw,'email')?raw.email:existing.email),valid=!!email&&v6ContactEmailValid_(email),dnc=raw.doNotContact!=null||raw.dnc!=null?v6ContactBool_(raw.doNotContact||raw.dnc):v6ContactBool_(existing.doNotContact||existing.dnc),externalContact=v6ContactText_(raw.externalContactId||raw.salesforceContactId),resolvedSalesforceContactId=v6ContactIsSalesforceSource_(source)?(externalContact||v6ContactText_(existing.salesforceContactId)):v6ContactText_(existing.salesforceContactId),record=Object.assign({},existing,{contactId:contactId,accountId:accountId,externalSystem:source,externalContactId:externalContact||v6ContactText_(existing.externalContactId),salesforceContactId:resolvedSalesforceContactId,canonicalSalesforceIdStatus:resolvedSalesforceContactId?'RESOLVED':'UNRESOLVED',email:email,emailStatus:!email?'MISSING':valid?'VALID':'INVALID',doNotContact:dnc,status:v6ContactText_(raw.status||existing.status||'ACTIVE').toUpperCase(),sourceUpdatedAt:v6ContactText_(raw.sourceUpdatedAt||existing.sourceUpdatedAt),createdAt:v6ContactText_(existing.createdAt||now),updatedAt:now});
