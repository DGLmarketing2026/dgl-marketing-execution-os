...
- `v6AuraRunRetentionCycle` -> `v6AuraRunRetentionCycle_()` — the full automatic cycle: freshness gate -> evaluate -> AM CSV report -> persisted run summary. This is now what the six-hour scheduler trigger calls (`v6ScheduledOpportunityRefresh_`).
- `v6AuraRetentionRunSummary` -> `v6AuraRetentionRunSummary_({runId})` — reads back one persisted `MKT_RETENTION_RUN_SUMMARY` row (aggregate counts + `csvDriveFileId`).
