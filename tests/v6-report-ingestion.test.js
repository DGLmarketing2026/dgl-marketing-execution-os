...
assert(engine.includes('v6RefreshOpportunitiesFromReports_()'));assert(engine.includes('v6PipelineSyncSignals_'));assert(engine.includes("status:'REPORT_SOURCE_SYNCED'"));['totalSignals','eligibleAccounts','suppressedAccounts','byType','lastEngineRun'].forEach(field=>assert(engine.includes(field)));
const frontend=fs.readFileSync(path.join(root,'assets/js/marketing-opportunity-engine-v6.js'),'utf8'),index=fs.readFileSync(path.join(root,'index.html'),'utf8');assert(frontend.includes('raw?.groups'));assert(index.includes('marketing-opportunity-engine-v6.js?v=20260827-1'));
