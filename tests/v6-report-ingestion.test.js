...
const frontend=fs.readFileSync(path.join(root,'assets/js/marketing-opportunity-engine-v6.js'),'utf8'),index=fs.readFileSync(path.join(root,'index.html'),'utf8');assert(frontend.includes('raw?.groups'));assert(/marketing-opportunity-engine-v6\.js\?v=\d{8}-\d+/.test(index));
